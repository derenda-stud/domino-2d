...
unsigned int ultima_posizione(matrice_t *piano_gioco, unsigned int riga) {
    ...
    for (size_t j = piano_gioco->colonne - 1; j > 0; j--) {
        // Controlla che ci sia una posizione occupata
        if (piano_gioco->posizione[riga][j].cardine) {
...
bool mossa_legale(matrice_t *piano_gioco, coord_t *coordinata, tessera_t *tessera, comb_t *risultato) {
    ...
    int corrente = estremi_corrispondono(piano_gioco, *coordinata, *tessera, risultato);
    // Imposta il valore della variabile booleana a [0/1]
    risultato->rotazione = corrente - 1;
    // Quando sono in orizzontale, non ho corrispondenza, oppure sono arrivato all'ultima riga
    if (risultato->orientamento || !corrente || piano_gioco->righe == coordinata->riga + 1) {
        // Salva solamente il controllo della la riga corrente
        return corrente;
    }
    // Altrimenti controlla anche la corrispondenza della riga sottostante
    int sottostante = estremi_corrispondono(piano_gioco, (coord_t){coordinata->riga + 1, coordinata->colonna}, (tessera_t){tessera->destro, tessera->sinistro, tessera->speciale}, risultato);
    // Rimane la corrispondenza se viene rispettato l'ordine
    if (corrente == sottostante) return sottostante;
    // Per i casi verticali rimanenti non e' possibile posizionarla in verticale
    return false;
}
...
int estremi_corrispondono(matrice_t *piano_gioco, coord_t coordinata, tessera_t tessera, comb_t *risultato) {
    ...
    estremo_t *estremo = &piano_gioco->posizione[coordinata.riga][coordinata.colonna];
    // Se inserisco una speciale oppure entrambi gli estremi appartengono alla [0|0] mantieni l'ordine
    if (!(estremo - 1)->valore && !(estremo + risultato->orientamento + 1)->valore) return true;
    // La tessera non potra' combaciare se gli estremi presenti sono discordanti
    if ((estremo - 1)->cardine && (estremo + risultato->orientamento + 1)->cardine && (estremo - 1)->valore != (estremo + risultato->orientamento + 1)->valore) return false;
    ...
    if ((estremo - 1)->cardine) {
        ...
        *(risultato->adiacente) = (coord_t){coordinata.riga, coordinata.colonna - 1};
        // In caso di inserimento speciale mantieni l'ordine
        if (tessera.speciale) controllo_sx = 1;
    ...
    if ((estremo + risultato->orientamento + 1)->cardine) {
        ...
        *(risultato->adiacente) = (coord_t){coordinata.riga, coordinata.colonna + risultato->orientamento + 1};
        // In caso di inserimento speciale mantieni l'ordine
        if (tessera.speciale) controllo_dx = 1;
    ...
    return (controllo_sx | controllo_dx) % 3;
}
